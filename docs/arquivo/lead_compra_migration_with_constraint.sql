-- Migração do sistema Lead Compra com constraint única no email
...
CREATE TABLE IF NOT EXISTS public.lead_compra (
    ...
    email VARCHAR(255) NOT NULL UNIQUE, -- Email único para evitar duplicatas
...
-- Insert sample data com ON CONFLICT (agora funciona porque email é UNIQUE)
...
ON CONFLICT (email) DO NOTHING; -- Agora funciona porque email tem constraint UNIQUE
