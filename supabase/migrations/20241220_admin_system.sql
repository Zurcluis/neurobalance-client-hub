-- Criação do sistema administrativo para NeuroBalance
-- Data: 20/12/2024
...
-- Política para admins: apenas admins ativos podem ver outros admins
CREATE POLICY admin_select_policy ON admins
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM admin_access_tokens t
            JOIN admins a ON t.id_admin = a.id
            WHERE t.token = current_setting('request.jwt.claims', true)::json->>'token'
...
-- Política para tokens: apenas o próprio admin pode ver seus tokens
CREATE POLICY admin_tokens_select_policy ON admin_access_tokens
    FOR SELECT
    USING (
        id_admin IN (
            SELECT a.id FROM admin_access_tokens t
            JOIN admins a ON t.id_admin = a.id
            WHERE t.token = current_setting('request.jwt.claims', true)::json->>'token'
            AND t.is_active = true
            AND t.expires_at > NOW()
