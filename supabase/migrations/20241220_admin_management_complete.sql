-- Migration: Complete Admin Management System
-- Description: Creates tables and functions for managing administrative users and their access tokens
-- Date: 2024-12-20
...
CREATE INDEX IF NOT EXISTS idx_admin_tokens_expires ON public.admin_access_tokens(expires_at);
...
-- Insert initial admin data (optional - can be removed in production)
...
CREATE POLICY "Only admins can insert admins" ON public.admins
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.admin_access_tokens t
            JOIN public.admins a ON t.admin_id = a.id
...
-- RLS Policies for admin_access_tokens table
CREATE POLICY "Admins can view all tokens" ON public.admin_access_tokens
    FOR SELECT USING (true);

CREATE POLICY "Only admins can manage tokens" ON public.admin_access_tokens
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.admin_access_tokens t
            JOIN public.admins a ON t.admin_id = a.id
...
GRANT SELECT ON public.admin_access_tokens TO authenticated, anon;
...
GRANT EXECUTE ON FUNCTION public.revoke_admin_token TO authenticated;
GRANT EXECUTE ON FUNCTION public.cleanup_expired_admin_tokens TO authenticated;
...
COMMENT ON TABLE public.admins IS 'Table storing administrative users information';
COMMENT ON TABLE public.admin_access_tokens IS 'Table storing access tokens for administrative users';
COMMENT ON FUNCTION public.create_admin_token IS 'Creates a new access token for an admin user';
COMMENT ON FUNCTION public.validate_admin_token IS 'Validates an admin access token and returns user info';
COMMENT ON FUNCTION public.revoke_admin_token IS 'Revokes an admin access token';
COMMENT ON FUNCTION public.cleanup_expired_admin_tokens IS 'Deactivates expired tokens';
COMMENT ON FUNCTION public.get_admin_permissions IS 'Returns permissions array based on admin role';
COMMENT ON FUNCTION public.update_admin_last_login IS 'Updates the last login timestamp for an admin';
